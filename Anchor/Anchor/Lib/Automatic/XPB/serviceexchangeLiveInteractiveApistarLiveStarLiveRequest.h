...
@interface serviceexchangeLiveInteractiveApistarLiveStarLiveRequest : GProtoBufferMessage

    //创建pb对象,构建数据
    +(serviceexchangeLiveInteractiveApistarLiveStarLiveRequest*)createRequest;

    //定义pb请求结构
    @property(strong,nonatomic) serviceexchangeStarLiveRequest*  request;
