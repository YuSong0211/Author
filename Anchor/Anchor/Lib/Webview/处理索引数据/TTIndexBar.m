...
#define FontName [UIFont fontWithName:@"PingFangSC-Regular" size:15]
// 防止循环引用
...
- (void)toSelectTitle:(CGPoint)touchPoint
{
    ...
    [_labelArr enumerateObjectsUsingBlock:^(__kindof UILabel * _Nonnull subview, NSUInteger idx, BOOL * _Nonnull stop) {
        if(touchPoint.y < CGRectGetMaxY(subview.frame)) {
            weakSelf.preLabel.backgroundColor = [UIColor clearColor];
            weakSelf.preLabel.textColor = weakSelf.textColor;
            subview.backgroundColor = weakSelf.selectedBackgroundColor;
            subview.textColor = weakSelf.selectedTextColor;
            subview.font = FontName;
            weakSelf.preLabel = subview;
            index = idx;
            title = subview.text;
            *stop = YES;
        }
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _indexLabel.font = [UIFont fontWithName:@"BebasNeue-Regular" size:16];
