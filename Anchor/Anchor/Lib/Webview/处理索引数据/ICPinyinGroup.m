...
+(NSDictionary *)group:(NSArray *)datas key:(NSString *)key{
    ...
    for (id model in datas) {
        ...
        // 再次检索当前的位置
        sectionNumber = [ICPinyinGroup conversionUpString:[[model valueForKey:key] description] currentIndex:sectionNumber];
    ...
    for (id model in datas) {
        ...
        //NSString *firstCharString = [ICPinyinGroup conversionUpString:[[model valueForKey:key] description] charString:[[NSString stringWithFormat:@"%C",firstChar]uppercaseString]];
    ...
    NSMutableArray * subarray = [[NSMutableArray alloc]initWithArray:charSortArray];
    
    // 数组中是否包含#
    if ([subarray containsObject:@"#"]) {
        NSInteger objectIndex = [subarray indexOfObject:@"#"];
        // 是否为第一位
        if (objectIndex == 0) {
            // 移除并放在最后一位
...
//  去除特殊字符呢
//    if (![subarray containsObject:@"ABCDEFGHIJKLMNOPQRSTUVWXYZ"]) {
//        if (subarray.count != 0) {
//            NSString * subString = subarray[0];
...
//    }
    ...
    return @{LEOPinyinGroupCharKey:subarray,
...
+(NSInteger)conversionUpString:(NSString *)firstString currentIndex:(NSInteger)index{
    if (firstString.length>0) {
        NSString *firstCharStr = [[firstString uppercaseString] substringToIndex:1];
        if ([firstCharStr isEqualToString:@"长"]) {
            return 2;
        }else if ([firstCharStr isEqualToString:@"重"]){
            return 2;
        }else{
            return index;
        }
...
+(NSString *)conversionUpString:(NSString *)firstString charString:(NSString*)charString{
