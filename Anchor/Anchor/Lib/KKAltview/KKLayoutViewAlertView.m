...
@implementation KKLayoutViewAlertView
...
+(void)showAltLayoutViewWithConfigModel:(KKAltStyleConfig *)viewConfig  WithClickBlock:(alertViewClickBlock)alertBlock{
    ...
    backView.backgroundColor = KKRGBA(0, 0, 0, 0.5);
    // 根视图指向
...
//    viewConfig.content = str;
    ...
    // 内容
    UILabel * contentLab = [[UILabel alloc]initWithFrame:CGRectMake(10, CGRectGetMaxY(titleLab.frame)+15, w-20, rect.size.height)];
...
+(void)dismiss{
    for (UIWindow *window in [[UIApplication sharedApplication] windows]) {
        UIView * view = [window viewWithTag:13003];
        if (view) {
...
@implementation KKAltShowLayoutViewView
