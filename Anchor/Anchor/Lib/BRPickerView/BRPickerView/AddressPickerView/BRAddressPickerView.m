...
- (void)loadData {
    ...
    NSString *path=[mainBundleDirectory stringByAppendingPathComponent:@"webviewAddress.json"];
    NSURL *url=[NSURL fileURLWithPath:path];
    NSData *data = [[NSData alloc] initWithContentsOfURL:url];
    NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
    
    NSArray *arrData = dic; // [NSArray arrayWithContentsOfFile:filePath];
...
- (void)showWithAnimation:(BOOL)animation {
    ...
    if ([self.defaultSelectedArr[0] isKindOfClass:[NSString class]]) {
        ...
        // 1
        for (int i = 0; i<self.addressModelArr.count; i++) {
            BRProvinceModel * model = self.addressModelArr[i];
            if ([model.name isEqualToString:self.defaultSelectedArr[0]]) {
                recordRowOfProvince = i;
                // 2
                for (int j = 0; j<model.city.count; j++) {
                    BRCityModel * city = model.city[j];
                    if ([city.name isEqualToString:self.defaultSelectedArr[1]]) {
                        recordRowOfCity = j;
                        // 3
                        for (int k = 0; k<city.town.count; k++) {
                            BRTownModel * townName = city.town[k];
                            if ([townName.name isEqualToString:self.defaultSelectedArr[2]]) {
                                recordRowOfTown = k;
                                break;;
                            }
                        }
