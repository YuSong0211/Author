...
- (UIImage *)getPlaceholdImageForModel:(LBScrollViewStatusModel *)model {
    ...
    if (mgr.placeholdImageCallBackBlock) {
        ...
        if (!placeholdImage) {
            ...
            NSString *keyStr = [KKTool stringInterceptionConversion:imageUrl];
            SDImageCache* cache = [SDImageCache sharedImageCache];
            //此方法会先从memory中取。
            UIImage *image =[cache imageFromCacheForKey:keyStr];
            if (image== nil) {
                
                NSString *imageUrl1 = [NSString stringWithFormat:@"%@",model.easyUrl];
                ...
                if (image1== nil) {
                    ...
                    placeholdImage = [KKTool decodeResourceFileImageName:@"icon_community_photo"];
                }else{
                    self.loadingView.hidden = NO;
                    placeholdImage = image1;
                }
            }else{
...
-(UIImage *)getPlaceholdImageErrorImageForModel:(LBScrollViewStatusModel *)model {
    ...
    if (mgr.placeholdImageCallBackBlock) {
        ...
        if (!placeholdImage) {
            ...
            if (image== nil) {
                ...
                if (image1== nil) {
                    self.loadingView.hidden = NO;
//                    placeholdImage = [UIImage imageWithColor:[KKColor getColor:appBgColor]];
                    placeholdImage = [KKTool decodeResourceFileImageName:@"icon_community_photo"];
                }else{
                    self.loadingView.hidden = NO;
                    placeholdImage = image1;
...
//                    image = [self getPlaceholdImageErrorImageForModel:model];
...
//    UIView *currentView  = mgr.imageViews[mgr.currentPage];
    ...
    self.oldFrame = [mgr.imageViewSuperView convertRect:currentViewFrame
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        [UIView animateWithDuration:0.15 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            [KKPubTagVCModel shareToolGetManager].tap = nil;
        }];
    }];
}
