...
- (void)setupTitleButtons {
    ...
    if (self.configure.isStaticStyle) { // SGPageTitleView 静止样式
        CGFloat btnY = 0;
...
/** 根据下标值添加 图片 */
- (void)addImageForIndex:(NSInteger)index andimageNamed:(NSString *)image {
    ...
    badge.image = [UIImage imageNamed:image];
    badge.tag = 2018 + index;
    [btn addSubview:badge];
}

/** 根据下标值添加 badge */
- (void)addBadgeForIndex:(NSInteger)index andNumber:(NSString *)number {
    UIButton *btn = self.btnMArr[index];
...
- (void)removeBadgeForIndex:(NSInteger)index {
    UIButton *btn = self.btnMArr[index];
    [btn.subviews enumerateObjectsUsingBlock:^(__kindof UILabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.tag != 0) {
