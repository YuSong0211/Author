...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        _scrollView = [[KKScrollView alloc] init];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (_startOffsetX != offsetX && !self.isNOSuperlifeCycle) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_isAnimated == YES && self.isScroll == NO) {
        return;
    }
    // 1、定义获取需要的数据
    CGFloat progress = 0;
...
- (void)setPageContentScrollViewCurrentIndex:(NSInteger)currentIndex {
    ...
    // 2、切换子控制器的时候，执行上个子控制器的 viewWillDisappear 方法
    if (self.previousCVC != nil && _previousCVCIndex != currentIndex &&!self.isNOSuperlifeCycle) {
        [self.previousCVC beginAppearanceTransition:NO animated:NO];
    }
