...
#import "KKApplePay.h"
@interface KKApplePay()<SKProductsRequestDelegate,SKPaymentTransactionObserver>
{
    NSString *_purchID;
    NSString *_prepeID;
...
+ (instancetype)shareIAPManager {
    static KKApplePay  *IAPManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        IAPManager = [[KKApplePay  alloc] init];
    });
    return IAPManager;
}
...
- (void)handleActionWithType:(IAPPurchType)type data:(NSData *)data{
    switch (type) {
        case IAPPurchSuccess:
            _handle(IAPPurchSuccess,data);
            YMLog(@"购买成功");
            break;
        case IAPPurchFailed:
            YMLog(@"购买失败");
            if(_handle){
                _handle(IAPPurchFailed,data);
            }
            break;
        case IAPPurchCancel:
            YMLog(@"用户取消购买");
            if(_handle){
                _handle(IAPPurchCancel,data);
            }
            break;
        case IAPPurchVerFailed:
            _handle(IAPPurchVerFailed,data);
            YMLog(@"订单校验失败");
            break;
        case IAPPurchVerSuccess:
            _handle(IAPPurchVerSuccess,data);
            YMLog(@"订单校验成功");
            break;
        case IAPPurchNotArrow:
            _handle(IAPPurchNotArrow,data);
            YMLog(@"不允许程序内付费");
            break;
        default:
            break;
...
//    }
    ...
    NSDictionary *dict = @{@"data":receipt_data,@"transaction_id":transaction_id,@"goodsId":goodsId};
    
    NSMutableArray *arr = [NSMutableArray arrayWithArray:[KKUserInfo share].appPayIDArray];
    [arr addObject:dict];
    [KKUserInfo share].appPayIDArray = arr;
    serviceucPaymentAppleApiverifyReceiptPaymentAppleVerifyReceiptRequest *request = [serviceucPaymentAppleApiverifyReceiptPaymentAppleVerifyReceiptRequest createRequest];
    serviceucIdEncode *encodeid = [[serviceucIdEncode alloc]init];
    encodeid.id_p = _prepeID;
    request.request.prepayId = encodeid;
    request.request.transactionId = transaction_id;
    request.request.receipt = receipt_data;
    [[NetWorkMgr createMgrWithMessage:request]sendRequest:^(serviceucPaymentAppleVerifyReceiptResponse *  _Nonnull message) {
        _handle(0,nil);
        NSMutableArray *arr = [NSMutableArray arrayWithArray:[KKUserInfo share].appPayIDArray];
        ...
        [arr removeLastObject];
        _handle(IAPPurchVerFailed,nil);
        WS
        [KKHub showMtTostWithTitle:@"提示" WithText:[NSString stringWithFormat:@"服务器订单验证失败"] WithBtnText:@"重试" WithClickBlock:^(NSInteger index) {
                  SS
                  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                      [self retquestApplePay:dict[@"data"] transaction_id:dict[@"transaction_id"] goodsID:dict[@"goodsId"]];
                  });
