...
+ (void)initialize {
    ...
    appearance.titleColor =[KKColor getColor:appMainTextColor];
...
- (void)czh_setUpView {
    ...
    wholeContainerView.backgroundColor = [KKColor getColor:colorDialogBg];
...
//    NSAssert(self.title.length>0 || self.message.length >0 || self.itemArray.count > 0 || self.cancelItemArray.count > 0, @"please add a title or a message or a item");
    ...
    if (self.animationStyle == CZHAlertViewAnimationStyleDefault) {
        ...
        [UIView animateWithDuration:0.12 delay:0.05 options:UIViewAnimationOptionCurveEaseIn animations:^{
            self.backgroundColor = CZHRGBColor(0x000000, 0.3);
...
- (void)czh_calculateFrameForAlert {
    ...
    CGFloat wholeContainerViewW = ScreenWidth;
    CGFloat wholeContainerViewX = (self.czh_width - wholeContainerViewW) * 0.5;
    CGFloat wholeContainerViewH = 0;

    CGFloat buttonContainerViewX = 15;
    CGFloat buttonContainerViewY = 16.5;
    ...
    if (self.itemArray.count > 0) {
        ...
        firstLine.backgroundColor = [UIColor colorWithRed:37/255.0f green:39/255.0f blue:47/255.0f alpha:1].CGColor;
        ...
        for (NSInteger i = 0; i < itemCount; i++) {
            ...
            CGRect buttonF = CGRectMake(buttonX, buttonY, buttonW - 30, buttonH);
    ...
    _buttonContainerView.frame = CGRectMake(buttonContainerViewX, buttonContainerViewY, buttonContainerViewW - 15, buttonContainerViewH);
    [_buttonContainerView cornerAllCornersWithCornerRadius:CZH_ScaleWidth(7.5)];
    
    wholeContainerViewY = (self.czh_height - wholeContainerViewH) * 0.5;
    _wholeContainerView.frame = CGRectMake(0, wholeContainerViewY, ScreenWidth, wholeContainerViewH);
...
- (void)czh_calculateFrameForActionSheet {
    ...
    if (itemCount + cancelItemCount > 0) {
        ...
        for (NSInteger i = 0; i < itemCount; i++) {
            ...
            CALayer *firstLine = [CALayer layer];
            firstLine.backgroundColor = ([KKColor getColor:colorDialogBg]).CGColor;
            [_buttonContainerView.layer addSublayer:firstLine];
        ...
        wholeContainerViewH = buttonContainerViewH + 16.5 + (KK_BOTTOM_SECURITY_HEIGHT == 34 ? 20 : -10);
        ...
        if (cancelItemCount > 0) {
            ...
            CGFloat buttonX = 15;
            CGFloat buttonY = buttonContainerViewH + CZH_ScaleWidth(25);
            CGFloat buttonW = buttonContainerViewW;
            CGFloat buttonH = buttonHeight;
            CZHWeakSelf(self);
            CZHAlertButton *button = [CZHAlertButton czh_alertbuttonWithTitle:item.title titleColor:item.titleColor titleFont:item.titleFont backgroundColor:item.backgroundColor buttonHandler:^(CZHAlertButton *button) {
            ...
            CGRect buttonF = CGRectMake(buttonX, buttonY, buttonW - (buttonX * 2), buttonH);
            button.frame = buttonF;
            
            [_wholeContainerView addSubview:button];
            [button cornerAllCornersWithCornerRadius:CZH_ScaleWidth(7.5)];
            
            wholeContainerViewH = CGRectGetMaxY(buttonF) + marginHeight  + 16.5 + (KK_BOTTOM_SECURITY_HEIGHT == 34 ? 20 : -10);
        } else {
            
            wholeContainerViewH = buttonContainerViewH + marginHeight  + 16.5 + (KK_BOTTOM_SECURITY_HEIGHT == 34 ? 20 : -10);
    ...
    _buttonContainerView.frame = CGRectMake(buttonContainerViewX, buttonContainerViewY, buttonContainerViewW - (buttonContainerViewX * 2) , buttonContainerViewH);
    [_buttonContainerView cornerAllCornersWithCornerRadius:CZH_ScaleWidth(7.5)];
