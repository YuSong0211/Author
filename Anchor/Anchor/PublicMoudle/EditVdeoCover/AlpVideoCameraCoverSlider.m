...
- (void)setup {
    ...
    self.rangeThumbView.layer.borderWidth = 3.0;
    self.rangeThumbView.layer.borderColor = KKRGBA(36, 246, 201, 1).CGColor;
...
- (void)updateRangeWithTouch:(UITouch *)touch {
    ...
    self.rangeThumbView.transform =  CGAffineTransformMakeScale(1.35, 1.35);
    // 当前手指所在的位置
    CGPoint point = [touch locationInView:self];
    // 让thumb的中心点x = 手指所在的点
    CGFloat currentX = point.x;
    // 限制手指拖动的最左侧和最后侧
    currentX = MAX(self.rangeThumbView.frame.size.width*0.5, MIN(currentX, self.frame.size.width - self.rangeThumbView.frame.size.width * 0.5)+((self.rangeThumbView.frame.size.width*1.35) - self.rangeThumbView.frame.size.width));
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    self.rangeThumbView.transform = CGAffineTransformIdentity;
    if (self.endTrackingWithTouchBlock) {
        self.endTrackingWithTouchBlock();
    }
}
