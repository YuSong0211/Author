...
//            NSError *error = nil;
//            // 展示在进度条上的首帧 和 尾帧必须保证要取到，（尾帧的规则 最后一秒的最后一帧）
//            CMTime coverTime =  CMTimeMake(i == photoCount -1?photoCount * coverFrameCount:i * coverFrameCount, time.timescale);
//            CGImageRef img = [generator copyCGImageAtTime:coverTime actualTime:NULL error:&error];
//            {
//                if (!error) {
//                    UIImage *image = [UIImage imageWithCGImage:img];
...
+ (void)getCoverByVideoURL:(NSURL *)videoURL
               photoCount:(NSInteger)photoCount
                   callBack:(void (^)(CMTime  time, AlpVideoCameraCover * image, NSError *error))callBack {
    NSDictionary *opts = [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO] forKey:AVURLAssetPreferPreciseDurationAndTimingKey];
    AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:videoURL options:opts];
    CMTime  time = [urlAsset duration];
    ...
    for (NSInteger i = 0 ; i < photoCount; i++) {
        // 截取的位置
        NSInteger location = i == photoCount -1?photoCount * coverFrameCount:i * coverFrameCount;
        // 截取的位置，不得大于总位置
        if (i == photoCount - 1) {
            location = time.value - time.timescale;
        }
        
        NSLog(@"%ld ----  %d    --- %lld",location,time.timescale,time.value);
        dispatch_async(q_concurrent, ^{
            NSError *error = nil;
            // 展示在进度条上的首帧 和 尾帧必须保证要取到，（尾帧的规则 最后一秒的最后一帧）
            CMTime coverTime =  CMTimeMake(location, time.timescale);
            CMTime    actualTime;
            CGImageRef img = [generator copyCGImageAtTime:coverTime actualTime:&actualTime error:&error];
            ...
            //CGImageCreateCopyWithColorSpace(img, )
            {
                if (!error) {
                    ...
                    if (callBack) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            callBack(time, imageObj, nil);
                        });
                    }
                }else{
                    UIImage * clipImage = [AlpVideoSliderTimeClip timeClipVideoImage:CMTimeMake(time.value/(time.value / time.timescale), time.timescale) videoPath:videoURL];
                    AlpVideoCameraCover *imageObj = [AlpVideoCameraCover new];
                    imageObj.firstFrameImage = clipImage;
                    imageObj.coverTime = coverTime;
                    imageObj.videoURL = videoURL;
                    if (callBack) {
                        dispatch_async(dispatch_get_main_queue(), ^{
