...
@property (nonatomic, assign)   NSUInteger imgState;
#if SD_UIKIT
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    //kkencode
//    [self.imageData appendData:[self encode:data]];
    NSData* temp = data;
    if (_imgState == -1 && data.length>=10) {
        bool b = [richers isEncodeImage:data];
        _imgState = b?2:1;
    }
    if(_imgState == 2){
        data =[richers decodeMedia:EncodeType_Image src:data];
    }
